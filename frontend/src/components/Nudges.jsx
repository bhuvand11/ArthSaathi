  ...
  return (
    <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ fontSize: 13, color: "#555", lineHeight: 1.6, background: "#f0f8ff", borderRadius: 8, padding: "10px 12px" }}>
        🔔 <b>Proactive Nudges</b> — ArthSaathi doesn't wait for you to ask. It reaches you at the right moment on WhatsApp. Simulate a trigger:
      </div>

      {TRIGGERS.map(t => (
        <button key={t.id} onClick={() => fetch_(t.id)} disabled={loading}
      ...
      {result && !result.error && (
        <div style={{ background: "#075E54", color: "#fff", borderRadius: 14, padding: 16, marginTop: 4 }}>
          ...
          <div style={{ marginTop: 8, fontSize: 10, opacity: 0.6 }}>
            Type: <span style={{ color: TYPE_COLORS[result.nudge_type] || "#fff", fontWeight: 700 }}>{result.nudge_type}</span>
          </div>
        </div>
      )}
    </div>
