...
const SAMPLE_GOALS = [
  "I want to buy a new bike worth Rs 80,000",
  "I want to go on a trip to Goa with friends",
  "I want to buy a new smartphone for Rs 25,000",
];
    ...
    <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #1a1a2e, #16213e)", borderRadius: 12, padding: "16px 14px", color: "#fff", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 12, opacity: 0.8, marginTop: 4 }}>
          See two futures. Hear from your future self. Choose wisely.
        </div>
      </div>
      ...
      <div>
        ...
        <textarea value={goal} onChange={e => setGoal(e.target.value)} rows={3}
          placeholder='Example: "I want to buy a bike worth Rs 80,000"'
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <div style={{ background: "#FFF3E0", borderRadius: 12, padding: 12, border: `2px solid ${result.recommended_choice === "now" ? "#43A047" : "#FF7043"}` }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 12, textAlign: "center", marginTop: 4 }}>
                {result.scenario_now?.title}
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: "#555", lineHeight: 1.6 }}>
                ...
                <div>💰 Savings after 6mo: <b style={{ color: result.scenario_now?.savings_after_6_months > 0 ? "#1D9E75" : "#D32F2F" }}>
                  Rs {result.scenario_now?.savings_after_6_months?.toLocaleString()}
                </b></div>
                <div>⚠️ Debt risk: <b style={{ color: result.scenario_now?.debt_risk === "High" ? "#D32F2F" : result.scenario_now?.debt_risk === "Medium" ? "#E65100" : "#1D9E75" }}>
                  {result.scenario_now?.debt_risk}
                </b></div>
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: "#333", fontStyle: "italic" }}>
                {result.scenario_now?.outcome}
              </div>
              {result.recommended_choice === "now" && (
                <div style={{ marginTop: 6, background: "#43A047", color: "#fff", borderRadius: 6, padding: "3px 6px", fontSize: 10, textAlign: "center", fontWeight: 700 }}>
                  ✅ RECOMMENDED
                </div>
              )}
            </div>
            ...
            <div style={{ background: "#E8F5E9", borderRadius: 12, padding: 12, border: `2px solid ${result.recommended_choice === "delay" ? "#43A047" : "#ddd"}` }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 12, textAlign: "center", marginTop: 4 }}>
                {result.scenario_delay?.title}
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: "#555", lineHeight: 1.6 }}>
                <div>💰 Save/month: <b>Rs {result.scenario_delay?.monthly_savings?.toLocaleString()}</b></div>
                <div>🏦 Savings after 6mo: <b style={{ color: "#1D9E75" }}>
                  Rs {result.scenario_delay?.savings_after_6_months?.toLocaleString()}
                </b></div>
                <div>⚠️ Debt risk: <b style={{ color: result.scenario_delay?.debt_risk === "High" ? "#D32F2F" : result.scenario_delay?.debt_risk === "Medium" ? "#E65100" : "#1D9E75" }}>
                  {result.scenario_delay?.debt_risk}
                </b></div>
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: "#333", fontStyle: "italic" }}>
                {result.scenario_delay?.outcome}
              </div>
          ...
          {showFuture && (
            <div style={{ background: "linear-gradient(135deg, #1a1a2e, #16213e)", borderRadius: 16, padding: 18, color: "#fff" }}>
              ...
              <div style={{ textAlign: "center", marginBottom: 14 }}>
                ...
                <div style={{ marginTop: 8, fontSize: 13, fontWeight: 700, color: "#e0d7ff" }}>
                  {result.future_self_name}
                </div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>
                  A message from 6 months in the future
                </div>
              </div>
              ...
              {/* The message */}
              <div style={{ fontSize: 14, lineHeight: 1.8, color: "#e0d7ff", fontStyle: "italic" }}>
              ...
              {/* CTA */}
              <div style={{ marginTop: 16, background: "rgba(255,255,255,0.1)", borderRadius: 10, padding: "10px 14px", textAlign: "center" }}>
                <div style={{ fontSize: 12, color: "#a0f0b0", fontWeight: 600 }}>
                  💚 Your future self is counting on you.
                </div>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 4 }}>
                  Start your savings plan today in the Chat tab.
                </div>
              </div>
            </div>
          )}
