...
IMPORTANT - Before answering, detect if the user shows any of these behavioral biases:
- FOMO: Fear of missing out on returns others are getting
- PRESENT_BIAS: Preferring now over future benefit
- LOSS_AVERSION: Excessive fear of losing money
- SOCIAL_PROOF: Doing something just because others are doing it
- OVERCONFIDENCE: Overestimating their financial knowledge

If you detect a bias, NAME it gently and give a debiasing response.
Respond in {req.user_profile.language} language if not English.
...
{{
  ...
  "bias_explanation": "one sentence why this bias applies" or null,
...
Reply language: {req.user_profile.language}
...
{{
  ...
  "plain_language_verdict": "2-3 sentences in simple terms",
  "what_to_do": "concrete action for the user",
  "real_alternative": "a legitimate alternative they can use instead"
}}
"""
...
Write a short, friendly nudge message (max 2 sentences).

Respond ONLY with this exact JSON:
{{
  "nudge_message": "the WhatsApp message",
  "nudge_type": "SAVE" or "WARN" or "EDUCATE" or "CELEBRATE",
  "suggested_action": "one tap action label"
}}
"""
...
Create two realistic financial scenarios comparing buying now vs delaying.
